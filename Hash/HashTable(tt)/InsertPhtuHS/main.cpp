...
int Insert(Hashtable& ht, Hocsinh x) {
    ...
    //###INSERT CODE HERE -
    // Nếu số phần tử đã đạt ngưỡng tải 0.7 * M thì không thêm nữa
    double loadFactor = double(ht.n) / ht.M;
    if (loadFactor >= 0.7) {
        return 0; // thất bại
    }

    // Tính chỉ số bucket bằng hàm băm
    int index = Hash(ht, x.Maso);

    // Thêm học sinh vào cuối danh sách bucket
    AddTail(ht.table[index], x);

    // Cập nhật số phần tử trong bảng băm
    ht.n++;
