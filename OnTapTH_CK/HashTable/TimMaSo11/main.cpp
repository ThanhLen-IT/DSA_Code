...
int Search(Hashtable ht, int maso, int &nprob) {
	...
	int m = ht.M;        // kích thước bảng
    long long h = maso % m;        // hàm băm chính
    int i = 0;
	nprob =0;

    while (i < m) {         // tối đa thử m lần
        int idx = (h + i * i) % m;  // vị trí tra cứu
        if (ht.table[idx].Maso == maso) {
			nprob = i+1;
            return idx;     // tìm thấy trả về chỉ số
        } else if (ht.table[idx].Maso == EMPTY) {
        	nprob = i+1;
            return -1;      // gặp ô trống → không có trong bảng
        }
        i++;
    }

    return -1;              // không tìm thấy sau m lần
}
